...
#include "Esri/ArcGISRuntime/Toolkit/OverviewMapController.h"
...
void Animate3DSymbols::componentComplete()
{
  ...
  // Prevent mouse events in map from changing the insetView
  connect(m_insetView, &MapQuickView::mouseDoubleClicked, this, [](QMouseEvent& mouseEvent)
  {
    mouseEvent.accept();
  });

  connect(m_insetView, &MapQuickView::mouseWheelChanged, this, [](QWheelEvent& wheelEvent)
  {
    wheelEvent.accept();
  });

  connect(m_insetView, &MapQuickView::mouseMoved, this, [](QMouseEvent& mouseEvent)
  {
    mouseEvent.accept();
  });
  ...
  m_insetView->graphicsOverlays()->clear();
  m_insetView->graphicsOverlays()->append(mapOverlay);
...
void Animate3DSymbols::changeMission(const QString &missionNameStr)
{
  ...
  if (missionReady() && m_routeGraphic)
  {
    ...
    m_insetView->setViewpointAndWait(Viewpoint(m_routeGraphic->geometry()));
...
void Animate3DSymbols::setInsetView(QObject* view)
{
  m_insetView = qobject_cast<MapQuickView*>(view);

  if (!m_insetView)
    return;
}

QObject* Animate3DSymbols::insetView() const
{
  return qobject_cast<QObject*>(m_insetView);
}
...
void Animate3DSymbols::zoomMapIn()
{
  ...
  m_insetView->setViewpoint(Viewpoint((Point)m_routeGraphic->geometry(), m_insetView->mapScale() / m_mapZoomFactor));
}
